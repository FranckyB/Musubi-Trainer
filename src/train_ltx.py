    ...
    *,
    ...
    if do_train:
        ...
        if resolved_workers is None:
            resolved_workers = _recommended_ltx_dataloader_workers()
        else:
        ...
        disable_compile_for_fp8 = (
            ...
            and enable_fp8_dit
            and should_disable_compile_with_fp8("LTX")
        )
        compile_enabled = bool(enable_compile_optimizations and not disable_compile_for_fp8)
        if disable_compile_for_fp8:
            logger("  compile note: ignoring --compile because FP8 is enabled for LTX family")
        ...
        selected_network_module, is_lokr = _network_settings(network_type, "networks.lora_ltx2")
        network_lines = [
