"""Z-Image training module for Musubi-Trainer.
...
def run_steps_for_model(
    ...
    *,
    ...
    # ── Resolve model paths ─────────────────────────────────────────────────
    dit_path = require_model_file(runtime_config.dit, "Z-Image DiT")
    vae_path = require_model_file(runtime_config.vae, "Z-Image VAE")
    text_encoder_path = require_model_file(runtime_config.text_encoder, "Z-Image Text Encoder")
    ...
    if do_cache_latents:
        ...
        try:
            ...
            raise RuntimeError(
                f"Latent caching failed for Z-Image ({model_name}).\n"
                "Verify Z-Image VAE path in Settings.\n"
                f"Details: {exc}"
            ) from exc
    ...
    if do_cache_text:
        ...
        try:
            ...
            raise RuntimeError(
                f"Text caching failed for Z-Image ({model_name}).\n"
                "Verify Z-Image Text Encoder path in Settings.\n"
                f"Details: {exc}"
            ) from exc
    ...
    if do_train:
        ...
        if resolved_workers is None:
            resolved_workers = _recommended_zimage_dataloader_workers()
        else:
        ...
        disable_compile_for_fp8 = (
            ...
            and enable_fp8_dit
            and should_disable_compile_with_fp8("Z-Image")
        )
        compile_enabled = bool(enable_compile_optimizations and not disable_compile_for_fp8)
        if disable_compile_for_fp8:
            logger("  compile note: ignoring --compile because FP8 is enabled for Z-Image family")
        ...
        if generate_training_args_only:
            ...
            launch_args = build_config_file_command(musubi_python, "zimage_train_network.py", train_config_path)
            ...
            try:
                ...
                raise RuntimeError(
                    f"Training launch failed for Z-Image ({model_name}).\n"
                    "Verify Settings > Z-Image > DiT, VAE, and Text Encoder paths.\n"
                    f"Details: {exc}"
                ) from exc
