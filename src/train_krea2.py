"""Krea2 training module for Musubi-Trainer.
...
Notes:
  - Training runs on the RAW DiT checkpoint.
  - Defaults to ``--network_module networks.lora_krea2`` in LoRA mode.
"""
...
def _output_name_default(dataset_name: str) -> str:
    return f"{dataset_name}_Krea2"


def _recommended_krea2_dataloader_workers() -> int:
...
def _validate_krea2_transformers_compatibility(musubi_python: Path, musubi_dir: Path) -> None:
    probe_code = (
        ...
        "    import transformers\n"
        "    payload = {\n"
        "        'ok': bool(hasattr(transformers, 'Qwen3VLConfig')),\n"
        "        'version': str(getattr(transformers, '__version__', 'unknown')),\n"
        "    }\n"
        "except Exception as exc:\n"
        "    payload = {'ok': False, 'version': 'unknown', 'import_error': str(exc)}\n"
        "print(json.dumps(payload))\n"
    )

    try:
        probe = subprocess.run(
            [str(musubi_python), "-c", probe_code],
            cwd=musubi_dir,
            capture_output=True,
            text=True,
            check=False,
        ...
        raise RuntimeError(
            "Failed to validate Krea2 backend dependencies before text caching. "
            f"Could not run dependency probe: {exc}"
        ) from exc

    if probe.returncode != 0:
        details = (probe.stderr or probe.stdout or "").strip()
        raise RuntimeError(
            "Krea2 dependency check failed while probing transformers. "
            f"Details: {details or 'unknown probe error'}"
        )

    raw_stdout = (probe.stdout or "").strip()
    if not raw_stdout:
        raise RuntimeError("Krea2 dependency check failed: empty probe output from transformers validation.")
    ...
    try:
        ...
        raise RuntimeError(
            "Krea2 dependency check failed: could not parse transformers probe result. "
            f"Raw output: {raw_stdout[:200]}"
        ) from exc

    transformers_version = str(payload.get("version", "unknown"))
    if bool(payload.get("ok", False)):
        return

    import_error = str(payload.get("import_error", "")).strip()
    error_lines = [
        "Krea2 requires a transformers build that exposes 'Qwen3VLConfig', but it is not available in this backend environment.",
        f"Detected transformers version: {transformers_version}",
    ]
    if import_error:
        error_lines.append(f"Transformers import error: {import_error}")
    error_lines.extend(
        [
            "Fix: update backend dependencies (run Setup.bat from Musubi-Trainer root), then retry the job.",
            "Advanced/manual fix: activate the trainer venv and upgrade transformers to a version with Qwen3VLConfig support.",
        ]
    )
    raise RuntimeError("\n".join(error_lines))
    ...
    *,
    ...
    if do_cache_latents:
        ...
        try:
            ...
            raise RuntimeError(
                f"Latent caching failed for Krea2 ({model_name}).\n"
                "Verify Krea2 VAE path in Settings.\n"
                f"Details: {exc}"
            ) from exc
    ...
    if do_cache_text:
        ...
        if cancel_requested and cancel_requested():
            raise TrainingCancelledError
        _validate_krea2_transformers_compatibility(musubi_python, musubi_dir)
        cache_text_args = [
            str(musubi_python),
            "krea2_cache_text_encoder_outputs.py",
            "--dataset_config", str(dataset_config),
        ...
        try:
            ...
            raise RuntimeError(
                f"Text caching failed for Krea2 ({model_name}).\n"
                "Verify Krea2 Text Encoder path in Settings.\n"
                f"Details: {exc}"
            ) from exc
    ...
    if do_train:
        ...
        max_data_loader_n_workers = _recommended_krea2_dataloader_workers()
        ...
        optimization_lines = [
            ...
            f"timestep_sampling = {toml_quote(str(timestep_sampling or 'krea2_shift'))}",
            f"weighting_scheme = {toml_quote(str(weighting_scheme or 'none'))}",
            f"max_train_steps = {train_steps_for_run}",
        ]
        discrete_flow_shift_str = (discrete_flow_shift or "").strip()
        if discrete_flow_shift_str:
            optimization_lines.append(f"discrete_flow_shift = {discrete_flow_shift_str}")
        ...
        if generate_training_args_only:
            ...
            launch_args = build_config_file_command(musubi_python, "krea2_train_network.py", train_config_path)
            ...
            try:
                ...
                raise RuntimeError(
                    f"Training launch failed for Krea2 ({model_name}).\n"
                    "Verify Settings > Krea2 > RAW DiT, VAE, and Text Encoder paths.\n"
                    f"Details: {exc}"
                ) from exc
